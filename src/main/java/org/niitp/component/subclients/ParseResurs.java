package org.niitp.component.subclients;
...
public class ParseResurs {
    ...
    public void setElement(Element element, CertificateData certificateData, String fileName,
                           List<ChannelAngle> channelAngleList,
                           List<ChannelRange> channelRangeList) {
        this.fdata = certificateData;
    ...
    private void parseChildNodes(NodeList childlist, ParamsDocument paramsDocument) {
        ...
        channel = "";
        String channelNum = "";
        if (fdata.getMetaData().size() > 1 && list.length <= 8) {
            channelNum = list[list.length - 1];
            channel = "канал " + channelNum;
        }
        if (fdata.getMetaData().size() > 1 && list.length == 9) {
            channelNum = list[list.length - 3];
            channel = "канал " + channelNum;
        }
...
//                childlist.item(i).getTextContent();
        ...
        for (int j = 0; j < fdata.getFileName().size(); j++) {
            ...
            else if (name.contains("_001_" + channelNum))
                fdata.getFileName().get(j).setLabel(channel);
        }
    }
    ...
    public void setAdditionalElement(Element element, CertificateData input, String fileName) {
        if (element.hasChildNodes()) {
            ...
            if (input.getMetaData().size() > 1) {
                ...
                for (int j = 0; j < input.getFileName().size(); j++) {
                    String name = input.getFileName().get(j).getData();
                    if (name.equals(cDataFileName))
                        input.getFileName().get(j).setLabel(channel);
                    else if (name.contains("_001_" + channelNum))
                        fdata.getFileName().get(j).setLabel(channel);
                }
